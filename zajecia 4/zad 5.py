...
def normalVectors(u,v):
    ...
    # w przypadku końców jajka, przypisz recznie wektory normalne
    if u == 0.0:
        return [0, -1, 0]
    elif u == 1.0:
        return [0, 1, 0]
...
def generateNormalVectors():
    ...
    for i, u in enumerate(u_values):
        for j, v in enumerate(v_values):
            if i >= N/2:
                normalVectorsMatrix[i, j][0] = normalVectors(u, v)[0] * -1
                normalVectorsMatrix[i, j][1] = normalVectors(u, v)[1] * -1
                normalVectorsMatrix[i, j][2] = normalVectors(u, v)[2] * -1
            else:
...
def render(time):
    ...
    if space_button_state:
        ...
        for i in range(N - 1):
            for j in range(N - 1):
                glVertex3fv(vertices[i,j])
                glVertex3f(vertices[i,j][0] + normalVectorsMatrix[i,j][0], vertices[i,j][1] + normalVectorsMatrix[i,j][1], vertices[i,j][2] + normalVectorsMatrix[i,j][2])
        glEnd() 
