...
def render(time):
    ...
    # połączenie ostatniego rzędu i ostatniej kolumny punktów na powierzchni jajka - inczej linie nie zamknęłyby się
    for i in range(N - 1):
        glVertex3fv(vertices[i, N - 1])
        glVertex3fv(vertices[i + 1, N - 1])

        glVertex3fv(vertices[N - 1, i])
        glVertex3fv(vertices[N - 1, i + 1])
    glEnd()
    glFlush()
