...
def compile_shaders():
    vertex_shader_source = """
        ...
        float rand2() {
            vec2 uv = vec2(gl_InstanceID, gl_VertexID) * 0.1;
            uv = fract(uv * vec2(12.9898, 78.233));
            float randNum = (sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453);
            return fract(randNum);
        }     
        ...
        float rand1() {
            ...
            int seed = int(gl_InstanceID) * 100 + int(gl_VertexID); // wartosc seed na podstawie instancji i wierzcholka
            seed = (a * seed + c) % m;
            return float(seed) / float(m);
        }

        void main(void) {
            vec2 offset = vec2(rand1(), rand1() * 2); 
            //vec2 offset = vec2(rand2(), rand2()); 

            // ustawienie zakresu deformacji
            vec2 deformation = mix(vec2(0.9, 1.1), vec2(1.1, 0.9), offset); 
            //vec2 deformation = mix(vec2(0.95, 1.05), vec2(1.05, 0.95), offset); 
            //vec2 deformation = mix(vec2(0.98, 1.02), vec2(1.02, 0.98), offset);

            gl_Position = P_matrix * V_matrix * M_matrix * (position - (gl_InstanceID % 10 * vec4(1, 0, 0, 0)) + (int(gl_InstanceID / 10) * vec4(0, 1, 0, 0)));
            
            gl_Position.x *= deformation.x;
            gl_Position.y *= deformation.y;
